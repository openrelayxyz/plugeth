...
func New(root common.Hash, db Database, snaps *snapshot.Tree) (*StateDB, error) {
	...
	// Begin PluGeth code injection 1/2
	if sdb.snap == nil {
...
func (s *StateDB) Commit(deleteEmptyObjects bool) (common.Hash, error) {
	...
	if s.snap != nil {
		...
		if parent := s.snap.Root(); parent != root {
			//begin PluGeth code injection 2/2
			pluginStateUpdate(root, parent, s.snapDestructs, s.snapAccounts, s.snapStorage, codeUpdates)
			if _, ok := s.snap.(*pluginSnapshot); !ok && s.snaps != nil {// This if statement (but not its content) was added by Plugeth; PluGeth code injection
